...
app = Flask(__name__)
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///iusmeta.db"
db = SQLAlchemy(app)
...
def author_page(author_username):
    author_page = User.query.filter_by(
        author_username=author_username
    ).first()  # Yazarı veritabanından sorgula
    if author_page:
        return render_template(
            "author-page.html", author_page=author_page
        )  # Yazarın sayfasını render_template ile döndür
    else:
